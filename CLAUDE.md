...
- **Routing**: `expo-router` file-based routing rooted at `src/app/`. `src/app/_layout.tsx` is the root layout; routes are `index.tsx` (Inicio), `tareas.tsx` (Tareas), and `gastos.tsx` (Gastos). `typedRoutes` is enabled, so route strings are type-checked — keep `href` values in sync with filenames.
...
- **Glass effect (`expo-glass-effect`)**: use `GlassView` for Apple's Liquid Glass surfaces (the prototype's floating chrome — tab bar, FAB). Real glass renders only on iOS 26+ in a dev build; everywhere else (older iOS, Android, web, Expo Go) it falls back to a plain `<View>` automatically, so it is safe to use unconditionally — no platform split needed. Gate glass-only styling on `isLiquidGlassAvailable()`. On iOS 26 the system `NativeTabs` is already Liquid Glass for free.
